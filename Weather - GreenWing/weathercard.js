  ...
  fetch(url)
      ...
      .then(data => {
          if (data.main && data.name && data.weather && data.weather.length > 0) {
              ...
              document.querySelector('.card__temp--current').innerText = Math.round(main.temp);
              document.querySelector('#min-temp').innerText = Math.round(main.temp_min);
              document.querySelector('#max-temp').innerText = Math.round(main.temp_max);
